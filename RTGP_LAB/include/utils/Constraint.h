...
public:
	...
	void satisfyConstraint()
	{
		...
		if(!p1->movable || !p2->movable)
			correctionRation = 1.0f; // Lets make it half that length, so that we can move BOTH p1 and p2.

		glm::vec3 correctionVectorHalf = correctionVector * correctionRation; // Lets make it half that length, so that we can move BOTH p1 and p2.
